from pomdp_py import BeliefState, BeliefDistribution_Histogram, Planner
import math
import time
import copy
...
class OOPOMCP(Planner):
    ...
    def print_preferred_actions(self, max_depth=None):
        self.print_preferred_actions_helper(self._tree, 0, max_depth=max_depth)

    def print_preferred_actions_helper(self, root, depth, max_depth=None):
        if max_depth is not None and depth >= max_depth:
            return
        best_child = None
        ...
        if best_child is not None and root[best_child] is not None:
            ...
            self.print_preferred_actions_helper(root[best_child], depth+1, max_depth=max_depth)
    ...
    def search(self, history):
        ...
        print("Preferred actions:")
        self.print_preferred_actions()        
