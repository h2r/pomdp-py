...
# from moos3d.planning.pomcp import POMCP, POMCP_Particles
        ...
        if action != 'detect':
            ...
            if self._gridworld.observable_at(next_robot_pose, self._sensor_params,
                ...
                if not target_id in next_observed_targets:
                    return 0.00001 # observable, but claiming it's not observed next.
                else:
                    return 0.99999  # observed next
            else:
                if target_id in next_observed_targets:
                    if target_id in current_observed_targets:
                        return 0.99999  # already observed. It's ok to not observe next.
                    else:
                        return 0.00001# not observable, but claiming it's observed next.
                else:
                    if target_id in current_observed_targets:
                        return 0.00001  # Impossible to go from observable to not observable
                    else:
                        return 0.99999  # doesn't observe now and doesn't observe next
...
def unittest():
    sensor_params = {
        ...
        'sigma_dist': 0.01,
        'sigma_bearing': 0.01}
    gridworld = MultiTargetGridWorld(world3)
    ...
    if solver == "random":
        ...
        num_particles = 5000
        oopomdp = initialize_oopomdp_particles(sensor_params, gridworld, robot_id, first_target_id, prior="GROUNDTRUTH",
                                               num_particles=num_particles)
        sys.stdout.write("Initializing Planner: POMCP...")
        planner = POMCP(oopomdp, max_depth=30, max_time=3., gamma=0.6, num_particles=num_particles,
                        rollout_policy=_rollout_policy, exploration_const=0.2,
                        observation_based_resampling=False)
        sys.stdout.write("done\n")
    else:
        # OOPOMCP
        oopomdp = initialize_oopomdp_histogram(sensor_params, gridworld, robot_id, first_target_id, prior="GROUNDTRUTH")
        sys.stdout.write("Initializing Planner: OOPOMCP...")
        planner = OOPOMCP(oopomdp, max_depth=100, max_time=1., gamma=0.6,
